...
def run_demo_step(objective, command_args, explanation=None, display_cmd=None):
    ...
    full_cmd = command_args + ["--exchange", "simulated", "--no-confirm"]
    ...
    executable_cmd = get_bitmango_executable()
    subprocess.run(executable_cmd + full_cmd, stderr=subprocess.DEVNULL)
    time.sleep(get_sleep_time(0.5))
