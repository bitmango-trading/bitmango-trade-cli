...
class TradingMixin:
    ...
    def close_all(self, args):
        """Closes ALL open positions for the current market type."""
        msg = get_message("trade.closing_position", symbol="ALL", exchange=self.exchange_name)
        display_message('action_start', msg)
        try:
            ...
            for pos in positions:
                size = float(pos.get('contracts', pos.get('amount', 0)))
                if size != 0:
                    symbol = pos['symbol']
                    side = 'sell' if size > 0 else 'buy'
                    abs_size = abs(size)
                    
                    display_message('info', get_message("account.closing_position_size", size=abs_size, symbol=symbol))
                    
                    params = {'reduceOnly': True}
                    if getattr(args, 'market_type', 'spot') == 'spot':
                        params = {}
                        
                    self.exchange.create_order(symbol, 'market', side, abs_size, None, params)
                    closed_count += 1
