...
class BinanceAccountPositions(BaseExchange):
    ...
    def entry(self, args):
        """
        Places an entry order (Spot/Futures).
        """
        ...
        # Binance Spot often needs specific timeInForce for limit orders
        if market_type == 'spot' and order_type == 'limit':
    ...
    def exit(self, args):
        ...
        market_type = getattr(self.args, 'market_type', 'spot')
        if market_type == 'spot':
            # Spot doesn't really have reduce-only, it's just a sell
            params = {}
            if order_type == 'limit':
                params['timeInForce'] = 'GTC'
