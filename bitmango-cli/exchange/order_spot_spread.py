        ...
        try:
            ...
            while free_balance < total_position_size and order_amount > 0:
                try:
                    ...
                    if order:
                        display_message('info', f"Limit buy order created for {order_amount} {symbol} at {target_price}")
                    ...
                    if free_balance >= total_position_size or not exchange.fetch_order(order['id'], symbol):
                        if not exchange.fetch_order(order['id'], symbol):
                            ...
                            if exchange.fetch_order(order['id'], symbol):
                                ...
                                display_message('info', "Order canceled due to not being filled within one second.")
                        break
                    ...
                    display_message('error', f"Error: {e}")
                    break

            if free_balance >= total_position_size:
                display_message('success', f"Successfully bought {total_position_size} {symbol}.")
            else:
                display_message('warning', "Unable to buy target position size due to price increase or orders not filled.")
