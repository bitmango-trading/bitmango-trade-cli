...
if (Test-Path "bin") {
    Write-Host "Detected Full-Source distribution (using 'bin/' directory)." -ForegroundColor Green
} else {
    Write-Host "Detected Source distribution." -ForegroundColor Green
}

function Install-Binary($Name) {
    $BinaryPath = Join-Path $InstallDir (Join-Path "bin" "$Name.exe")
...
"@
    ...
    } elseif (Test-Path (Join-Path $InstallDir "bitmango-cli")) {
...
uv run python "$InstallDir\bin\$Name" %*
"@
        $Content | Out-File -FilePath $ShimPath -Encoding ascii
    } else {
        Write-Error "Binary not found and no source code found for $Name"
    }
}
}
